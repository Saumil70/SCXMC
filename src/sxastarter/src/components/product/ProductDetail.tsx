...
import Giftcardform from 'components/product/GiftCardForm';
import {
  AddToCartReq,
  GiftCard,
  ProductDetailType,
...
const ProductDetail = () => {
  ...
  useEffect(() => {
    ...
    if (prodcutStore && search && prodcutStore.find((x) => x.Id === parseInt(search))) {
      ...
      setQuantity(1);
      setPrice(storedProduct?.ProductPrice?.PriceWithDiscount ?? '0');
      ...
      ProductService.getProductDetails(search)
        .then((res) => {
          ...
          setQuantity(1);
          setPrice(res.data.ProductPrice.PriceWithDiscount);
